...
class AttentionWalkLayer(torch.nn.Module):
    ...
    def forward(self, weighted_target_tensor, adjacency_opposite):
        ...
        temp=torch.abs(self.args.num_of_walks * weighted_target_matrix.shape[0] * loss_on_target + loss_opposite)
        temp2=torch.log(1-torch.sigmoid(torch.mm(self.left_factors, self.right_factors)))
