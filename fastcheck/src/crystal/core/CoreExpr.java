...
public abstract class CoreExpr {
	...
	private static CoreExpr rvalue(Expression e) {
		...
		if (e instanceof CastExpr) {
			...
			return getExpr(new CoreConstExpr(se.getType()));
		}
			...
			Type t = ExprType.typeOf(e); 
			if (be.getIntOp() == ArithmOps.PLUS && t.isPointer()) {
				CoreExpr size = new CoreConstExpr(t.getTargetType());
				cr = CoreBinaryExpr.newExpr(ArithmOps.TIMES, cr, size);
			}
	...
	private static CoreExpr lvalue(Expression e) {		
		if (e instanceof CastExpr) {
			...
			Type type = ExprType.typeOf(ae);
			Expression sub = 
				new ArithmExpr(ArithmOps.Times, ae.getSub(), 
						new SizeofExpr(type));
			CoreExpr csub = getExpr(rvalue(sub));
