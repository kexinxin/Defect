...
class GeneticHyperopt:
    def __init__(self, learner, X, y, fitness_func, maximize, num_folds=5,
                 pop_size=40, num_gen=5, elite_percent=0.1, competitiveness=0.4, mutation_prob=0.2, classfierParameterSize=0):
        ...
        self.classfierParameterSize=classfierParameterSize
        self.params = []
    ...
    def _to_param_classfier_dict(self, ind_params):
        param_dict = {}
        for i in range(self.classfierParameterSize):
            param_dict[self.params[i].name] = ind_params[i]
        return param_dict

    def _to_param_sample_dict(self, ind_params):
        param_dict = {}
        for i in range(self.classfierParameterSize,len(self.params)):
            param_dict[self.params[i].name] = ind_params[i]
        return param_dict


    def _evaluate_individual(self, ind_params):
        param_dict = self._to_param_classfier_dict(ind_params)
        learner_obj = self.learner(**param_dict)
        sampe_dict=self._to_param_sample_dict(ind_params)
        score = 0
        for train_ind, val_ind in self.indices:
            x_train, y_train = IMAHAKIL(**sampe_dict).fit_sample(self.X[train_ind, :], self.y[train_ind])
    ...
    def evolve(self):
        ...
        for i in range(self.num_gen):
            ...
            print("---")
            if self.maximize:
                resultfitness=-min(fitness)
            else:
                resultfitness=min(fitness)
        return self._to_param_dict(self.population[0]), resultfitness
