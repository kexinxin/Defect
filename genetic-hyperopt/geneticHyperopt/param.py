...
class CategoricalParam(Param):
    ...
    def DEmute(self, x, y, z, f, origin):
        return np.random.choice([c for c in self.categories if c != origin])

class ContinuousParam(Param):
    def __init__(self, name, min_limit=None, max_limit=None, is_int=False):
    ...
    def sample(self):
        ...
        value=np.random.uniform(self.min_limit,self.max_limit)
